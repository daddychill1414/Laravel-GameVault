...
<body class="min-h-screen bg-[--color-vault-dark] bg-tech-grid flex items-center justify-center p-4 antialiased selection:bg-[--color-vault-accent] selection:text-white relative overflow-hidden">
    ...
    <div class="w-full max-w-md relative z-10">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-xs font-mono tracking-widest text-[--color-vault-gold] uppercase mt-1">Create User Account</p>
        </div>
        ...
        <div class="glass-panel rounded-3xl p-8 shadow-2xl border border-white/10">
            <h2 class="text-xl font-bold text-white mb-6 tracking-tight">Sign Up for GameVault</h2>
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-4">
                ...
                {{-- Confirm Password --}}
                <div>
                    <label class="block text-xs font-mono uppercase tracking-wider text-[--color-vault-muted] mb-2" for="password_confirmation">
                        Confirm Password
                    </label>
                    <input type="password" name="password_confirmation" id="password_confirmation"
                        placeholder="Repeat your password" required
                        class="input input-bordered w-full bg-white/[0.03] border-[--color-vault-border] focus:border-[--color-vault-accent] focus:outline-none text-white rounded-xl placeholder:text-slate-600" />
                </div>

                {{-- Submit --}}
                <button type="submit"
                    class="btn w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white border-none shadow-lg shadow-purple-600/30 rounded-xl py-3 h-auto font-bold tracking-wide mt-3 hover-lift">
                    Create Account
                </button>
            </form>
